const { NotFoundError, ConflictError, PaystackPermanentError } = require("../error")
...
class DriverService{
    ...
    async createDriver(userId, data){
        ...
        let subAccountCode
        try {
            const result = await this.paystackGateway.createSubaccount(
            ...
            if(error instanceof PaystackPermanentError){
                await this.driverRepository.deleteById(driver.driverProfileId)
            }
            throw error
        }
